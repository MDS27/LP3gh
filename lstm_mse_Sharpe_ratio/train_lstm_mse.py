...
def build_model(input_shape, num_assets):
    model = keras.Sequential([
        ...
        layers.LSTM(64),
        layers.Dense(num_assets, activation="linear")  # прогноз доходностей
    ])
    return model
...
if __name__ == "__main__":
    ...
    print("Форма X_train:", X_train.shape)  # (samples, 50, features)
    print("Форма y_train:", y_train.shape)  # (samples, assets)
    ...
    model.save(os.path.join(MODEL_DIR, "lstm_mse_portfolio_model.keras"))
